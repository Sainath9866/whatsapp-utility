  ...
  async function act(msg: WhatsAppMessage) {
    ...
    if (!history) {
      ...
      try {
        ...
        window.open(buildWhatsAppUrl(latest), "_blank", "noopener,noreferrer");
        const [copied] = await Promise.all([copyMessage(latest.message), markSent(latest.id)]);
        setResult((previous) => ({ ...previous, messages: previous.messages.filter((row) => row.id !== latest.id), total: Math.max(0, previous.total - 1) }));
        setNotice(copied
          ? `Opened the latest message for ${latest.recipient_name}. If WhatsApp kept an old draft, select all and paste the copied text.`
          : `Opened the latest message for ${latest.recipient_name}. Your browser blocked the backup copy, so verify the text before sending.`);
        setRetry((value) => value + 1);
        onChanged();
      } catch (error) {
        setActionError(`Could not open WhatsApp: ${errorMessage(error)}`);
